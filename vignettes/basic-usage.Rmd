...
The `twoxtwo()` function is used in other package functions that allow users to directly perform data analysis *without* first creating a `twoxtwo` object.

Individual measures of effect (odds ratio, risk ratio, and risk difference) can be calculated directly. Each measure includes the point estimate and confidence interval based on the $\alpha$ specified and standard error around the estimate:
...
As with measures of effect, hypothesis tests (Fisher's exact test for count data and Pearson's $\chi^2$ test) can also be run *without* first creating a `twoxtwo` object:
...
All processing of exposure and outcome variables requires that both **must** have only two levels. By default, both variables are coerced to factors and reversed. The effect is that, as in the example presented above, a value of `TRUE` or `1` will be oriented as exposure or outcome "+" and a corresponding value of `FALSE` or `0` will be oriented as exposure or outcome "-". 

Anticipating that this logic may not satisfy all use-cases, the `twoxtwo()` constructor function is flexible enough to allow user-specified ordering via a named list passed to the "levels" argument:
